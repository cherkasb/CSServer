...
new const g_URL[] = "https://api.telegram.org/bot1406248091:AAFBshTYeoi_JG8TXUt6x9SmEaOnRftXj-4/sendmessage?chat_id=391617470&text=";
...
public message(id) {
    ...
    if(g_Start_Time <= CurHour && CurHour < g_End_Time) {
        ColorChat(id, CHATCOLOR_RED, "%l %l", "TG_TAG", "TG_SLEEP");
        return PLUGIN_HANDLED;
    }
    #endif

    if(g_flNextTime[id] > get_gametime()) {
        ColorChat(id, CHATCOLOR_RED, "%l %l", "TG_TAG", "TG_SPAM");
        return PLUGIN_HANDLED;
    }

    client_cmd(id,"messagemode Telegram");
    ColorChat(id, CHATCOLOR_GREEN, "%l %l", "TG_TAG", "TG_ENTER");
...
public CommandMessage(id) {
    ...
    ColorChat(id, CHATCOLOR_GREEN, "%l %l", "TG_TAG", "TG_OK");
