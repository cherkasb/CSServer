...
public plugin_init( )
{
	...
	g_pCvarSQLHost      = register_cvar( "amx_gag_sql_host",      "cshost.su"      );
	g_pCvarSQLUser      = register_cvar( "amx_gag_sql_user",      "mysql27062"      );
	g_pCvarSQLPass      = register_cvar( "amx_gag_sql_pass",      "e8jWUgrUZl"      );
	g_pCvarSQLDb        = register_cvar( "amx_gag_sql_db",        "mysql27062"      );
...
GagPlayer( id, iPlayer, iGagTime, iFlags )
{
	...
	show_activity( id, szAdmin, "Замутил %s %s! (%s)", szName, szInfo, szFrom );
	
	console_print( id, "Вы замутили ^"%s^" (%s) !", szName, szFrom );
	
	log_amx( "^"%s<%s>^" замутил ^"%s<%s>^" %s. (%s)", szAdmin, g_szAuthid[ id ], szName, g_szAuthid[ iPlayer ], szInfo, szFrom );
}
...
public CmdAddGag( const id, const iLevel, const iCid )
{
	if( !cmd_access( id, iLevel, iCid, 2 ) )
	{
		console_print( id, "Флаги: a - чат | b - коммандный чат | c - голосовой чат" );
		return PLUGIN_HANDLED;
	}
	...
	{
		console_print( id, "Неправильный steamid (%s). ^"STEAM_0:X:XXXXX^" - это необходимый формат", szArg );
		return PLUGIN_HANDLED;
	}
	...
	{
		console_print( id, "Игрок уже замучен!" );
		return PLUGIN_HANDLED;
	}
	
	if( GetAccessBySteamID( szArg ) & ADMIN_IMMUNITY )
	{
		console_print( id, "У игрока иммунитет!" );
		return PLUGIN_HANDLED;
	}
	...
	show_activity( id, szAdmin, "Рамутил отключённого пользователя <%s>! (%s)", data[ GAG_AUTHID ], szInfo, szFrom );
	
	console_print( id, "Вы замутили ^"%s^" (%s) !", data[ GAG_AUTHID ], szFrom );
	
	log_amx( "^"%s<%s>^" замутил неподключённого игрока ^"<%s>^" %s. (%s)", szAdmin, g_szAuthid[ id ], data[ GAG_AUTHID ], szInfo, szFrom );
...
public CmdUnGagPlayer( const id, const iLevel, const iCid )
{
	...
	if( szArg[ 0 ] == '@' && equali( szArg[ 1 ], "all" ) )
	{
		...
		console_print( id, "Вы размутили всех игроков!" );
		
		new szAdmin[ 32 ];
		get_user_name( id, szAdmin, 31 );
		
		show_activity( id, szAdmin, "Размутил всех игроков." );
		
		log_amx( "^"%s<%s>^" размутил всех игроков.", szAdmin, g_szAuthid[ id ] );
	...
	if( !iPlayer )
	{
		...
		{
			console_print( id, "Этот steamid не замучен!" );
			return PLUGIN_HANDLED;
		}
	...
	else
	{
		...
		{
			console_print( id, "^"%s^" не замучен!", szName );
			return PLUGIN_HANDLED;
		}
	}
	...
	show_activity( id, szAdmin, "Размутил %s.", szName );
	
	console_print( id, "Вы размутили ^"%s^" !", szName );
	
	log_amx( "^"%s<%s>^" размутил ^"%s<%s>^"", szAdmin, g_szAuthid[ id ], szName, g_szAuthid[ iPlayer ] );
			...
			if( is_user_connected( iPlayer ) )
			{
				if( !g_iMenuOption[ id ] )
				{
					...
					if( TrieGetCell( g_tArrayPos, g_szAuthid[ iPlayer ], iArrayPos ) )
					{
						...
						console_print( id, "Вы замутили ^"%s^" !", szName );
