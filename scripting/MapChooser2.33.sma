...
new MapFile[64], MapCycle[64], MapFile_Lines, MapCycle_Lines;
...
new const MapMenuCommand[] = { "/playedmaps" };
new const TimeLeftCommand[] = { "timeleft" };
new const NextMapCommand[] = { "/nextmap" };
new const AdminCommand[] = { "/startvote" };
new const RTVCommand[] = { "/rtv" };
new const NomCommand[] = { "/maps" };
new const CurrentMapCommand[] = { "/currentmap" };
...
public plugin_init()
{
	...
	formatex(MapCycle, 63, "%s/maps.ini", ConfigsDir);
	...
	{
		format(Cmd, charsmax(Cmd), "say %s", MapMenuCommand);
		register_clcmd(Cmd, "ShowMapMenu");
	}
	
	format(Cmd, charsmax(Cmd), "say %s", NextMapCommand);
	register_clcmd(Cmd, "ShowNextMap");
	
	format(Cmd, charsmax(Cmd), "say %s", CurrentMapCommand);
	register_clcmd(Cmd, "ShowCurrentMap");

	format(Cmd, charsmax(Cmd), "say %s", TimeLeftCommand);
	register_clcmd(Cmd, "ShowTimeLeft");
	
	format(Cmd, charsmax(Cmd), "say %s", AdminCommand);
	register_clcmd(Cmd, "StartVote");
	
	if(get_pcvar_num(Cvar_RTV) == 1)
	{
		format(Cmd, charsmax(Cmd), "say %s", RTVCommand);
		register_clcmd(Cmd, "RockTheVote");
	}
	
	if(get_pcvar_num(Cvar_Nomination) == 1)
	{
		format(Cmd, charsmax(Cmd), "say %s", NomCommand);
...
public RockTheVote(id)
{
	...
	if(PlayerRTV[id] || Voted || inProcess || !toRTV || Off == 1)
	{
		if(!toRTV)
		{
			if(RTVTime[2] > Time[0])
			{
				print_color(id, "%L", Lang, "RTV2", (RTVTime[1]+60)-Time[1]);
			}
			else
			{
				print_color(id, "%L", Lang, "RTV2", RTVTime[1]-Time[1]);
			}
		}
		
		if(PlayerRTV[id])
		{
			print_color(id, "%L", Lang, "RTV1");
		}
	...
	{
		print_color(0, "%L", Lang, "RTV3", (Num/get_pcvar_num(Cvar_RTVMin))-RTV);
	}
...
public ShowTimeLeft()
{
	if(get_pcvar_num(Cvar_Mod) == 1)
	{
		...
		{
			print_color(0, "!y%L!t !t-!y:!t-", Lang, "TL");
		}
		else
		{
			if(Time[2] > 0 && Time[1] > 0 && Time[0] > 0)
				print_color(0, "!y%L!t %d%!y:!t%02d!y:!t%02d", Lang, "TL", Time[2], Time[1], Time[0]);
			else if(Time[1] > 0 && Time[0] > 0)
				print_color(0, "!y%L!t %02d!y:!t%02d", Lang, "TL", Time[1], Time[0]);
			else
				print_color(0, "!y%L!t !t-!y:!t-", Lang, "TL");
		}
	}
	...
	{
		print_color(0, "!t%L", Lang, "RM", MaxRounds-Rounds);
	}
}
public ShowNextMap()
{
	...
	print_color(0, "!y%L!t %s", Lang, "NM", Next);
}
public ShowCurrentMap()
{
	print_color(0, "!y%L", Lang, "CM", MapName);
}
public Counter()
{
	if(MapCounter < 1)
...
public VoteMenuKeys(id, Key)
{
	...
	if(Key < get_pcvar_num(Cvar_MaxMaps) && Key > -1)
	{
		PlayerMap[id] = Key;
		PlayerVoted[id] = true;
		print_color(0, "%L", Lang, "PCHO", PlayerName, MapNames[Key]);
		MapVote[Key]++;
...
public MapKey(id, Menu, Item)
{
	...
	print_color(id, "%L", Lang, "MNP", MapDatas[0], MapDatas[1]);
}
public ShowNomMenu(id)
{	
	if(Nom >= get_pcvar_num(Cvar_MaxMaps)-1)
	{
		print_color(id, "%L", Lang, "NOMT");
		return;
	}
	if(AlreadyNom[id])
	{
		print_color(id, "%L", Lang, "NOMN");
		return;
	}
...
public NomKey(id, Menu, Item)
{
	...
	print_color(0, "%L", Lang, "NOMC", PlayerName, Data_1);
...
public RevoteMenuKeys(id, Key)
{
	...
	if(Key <= 2 && Key > -1)
	{
		...
		print_color(0, "%L", Lang, "PCHO", PlayerName, MapRevote[Key]);
		RevoteCounts[Key]++;
		if(get_pcvar_num(Cvar_ChooseSound) == 1)
		{
			client_cmd(0, "spk buttons/lightswitch2");
...
stock CheckVotes() {
	...
	if((MapVote[VoteNum_1]*100/AllVotes) >= get_pcvar_num(Cvar_VotePercentMin))
	{
		...
		if(get_pcvar_num(Cvar_Mod) == 1)
		{
			if(equali(Next, MapName))
			{
				...
				print_color(0, "%L", Lang, "MEXTEND1", get_pcvar_num(Cvar_Extend));
		...
		else if(get_pcvar_num(Cvar_Mod) == 0)
		{
			if(equali(Next, MapName))
			{
				print_color(0, "%L", Lang, "MEXTEND2", get_pcvar_num(Cvar_Extend));
	...
	else
	{
		...
		print_color(0, "%L", Lang, "RER", get_pcvar_num(Cvar_VotePercentMin));
	}	
