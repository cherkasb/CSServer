...
#define FLAG_ADMIN_BAN "d"
#define FLAG_VIP "r"
#define FLAG_PREMIUM "s"

new COLCHAR[3][2] = { "^x03"/*team col*/, "^x04"/*green*/, "^x01"/*white*/ }

//cvar pointers
new p_allchat, p_namecol, p_msgcol, p_hidestat, p_teamchat

//vars to check if message has already been duplicated
new alv_msguser, alv_str2[26], alv_str4[101]
new msg[200]
...
public col_changer( msg_id, msg_dest, rcvr )
{
	...
	if( equal( str2, "#Cstrike_Chat", 13 ) )
	{
		...
		if( !strlen( str3 ) )
		{
			...
			new msguser = get_msg_arg_int( 1 )
			
			new bool:is_team_msg = !bool:equal( str2, "#Cstrike_Chat_All", 17 )
			
			new msguser_team = get_user_team( msguser )
			new bool:is_msguser_spec = !bool:( 0 < msguser_team < 3 )
			
			new namecol = clamp( get_pcvar_num(p_namecol), 0, 2 )
			new msgcol = clamp( get_pcvar_num(p_msgcol), 0, 2 )
			
			new bool:same_as_last = bool:( alv_msguser == msguser && equal( alv_str2, str2 ) && equal( alv_str4, str4) )
			
			if( !same_as_last )
			{
				//Duplicate message once
				new allchat = clamp( get_pcvar_num( p_allchat ), 0, 2 )
				if( allchat == 2 ||  allchat == 1 )
				{
					if( !( is_team_msg && ( is_msguser_spec || is_team_msg && get_pcvar_num( p_teamchat ) == 0 ) ) )
					{
						//Don't duplicate if it's a spectator team message
						new flags[5], team[10]
						if( is_user_alive( msguser ) )
							flags = "bch"
						else
							flags = "ach"
						...
						if( get_user_flags( msguser ) & FLAG )
							buildmsg( msguser, is_msguser_spec, is_team_msg, msguser_team, namecol, msgcol, str4 ) //admin colored, by cvars
						else
							buildmsg( msguser, is_msguser_spec, is_team_msg, msguser_team, 0, 2, str4 ) //normal colors
			...
			if( get_user_flags( msguser ) & FLAG && ( namecol != 0 || msgcol != 2 ) )
			{//execute if msguser is admin and cols are not set to engine defaults
				if( !same_as_last )
					buildmsg( msguser, is_msguser_spec, is_team_msg, msguser_team, namecol, msgcol, str4 )
...
public buildmsg( msguser, is_msguser_spec, is_team_msg, msguser_team, namecol, msgcol, str4[ ] )
{
	new msguser_name[33]
	get_user_name( msguser, msguser_name, 32 )
	...
	if( get_pcvar_num( p_hidestat ) == 0 )
	{
		if (is_user_alive(msguser))
		{
			if (get_user_flags(msguser) & read_flags(FLAG_ADMIN_BAN))
				prefix = "^x04[Админ] "
			else if (get_user_flags(msguser) & read_flags(FLAG_PREMIUM))
				prefix = "^x04[Премиум] "
			else if (get_user_flags(msguser) & read_flags(FLAG_VIP))
				prefix = "^x04[VIP] "
		}
		else
		{
			if( is_msguser_spec )
				prefix = "^x01*Наблюдатель* "
			else if( !is_user_alive( msguser ) )
				prefix = "^x01*Мёртв* "
			else if (get_user_flags(msguser) & read_flags(FLAG_ADMIN_BAN))
				prefix = "^x04[Админ] "
			else if (get_user_flags(msguser) & read_flags(FLAG_PREMIUM))
				prefix = "^x04[Премиум] "
			else if (get_user_flags(msguser) & read_flags(FLAG_VIP))
	...
	if( is_team_msg )
	{
		if( is_msguser_spec )
			prefix = "^x01(Наблюдатель) "
		else if( msguser_team == 1 )
			add( prefix[strlen(prefix)-1], 29, "(Зомби) " )
		else if( msguser_team == 2 )
			add( prefix[strlen(prefix)-1], 29, "(Человек) " )
	}
	
	format( msg, 199, "%s%s%s :  %s%s",\
		strlen( prefix ) > 1 ? prefix : "",\
		COLCHAR[namecol], msguser_name, COLCHAR[msgcol], str4 )
