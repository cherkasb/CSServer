...
#define LANDMINE_OWNER			pev_iuser2 //EV_INT_iuser3

#define MODEL_MINE				"models/zombiehazard/landmine_pack.mdl"

#define MODEL_EXPLODE				"sprites/zombiehazard/MineExplode.spr"

#define SOUND_EXPLODE				"zombiehazard/landmineExplode.wav"
#define SOUND_ACTIVE				"zombiehazard/landmineActive.wav"
...
new cvar_mindamage, cvar_maxdamage, cvar_exploderadius, cvar_explodedelay
...
public plugin_init() 
{
	...
	cvar_mindamage = register_cvar("zp_landmine_mindamage", "800.0")
	cvar_maxdamage = register_cvar("zp_landmine_maxdamage", "1000.0")
	cvar_exploderadius = register_cvar("zp_landmine_exploderadius", "100.0")
	cvar_explodedelay = register_cvar("zp_landmine_explodedelay", "0.1")
...
{
	if (!zp_has_round_started() || zp_is_assassin_round() || zp_is_nemesis_round())
	{
		zp_setextraitemavailable(id, g_itemid_landmine, false)
		return (PLUGIN_HANDLED);
	}
	zp_setextraitemavailable(id, g_itemid_landmine, true)
...
public Spawn(iPlayer)
{
	...
	if (pev_valid(pEntity))
	{
		...
		set_pev(pEntity, LANDMINE_OWNER, iPlayer);
...
public HamHook_Think(const iEntity)
{
	...
	static iAttacker;
	iAttacker = pev(iEntity, LANDMINE_OWNER);
	
	static Float:OriginEnt[3];pev(iEntity, pev_origin, OriginEnt);
	
	static Float:iTime;
	pev(iEntity, pev_fuser1, iTime);
	...
	if (equal(Classname, MINE_CLASSNAME))
	{
		if (iTime  && iTime <= get_gametime() || !is_user_connected(iAttacker) || zp_get_user_zombie(iAttacker))
		{
			...
			while(((iVictim = fm_find_ent_in_sphere(iVictim, OriginEnt, get_pcvar_float(cvar_exploderadius))) != 0))
			{
				...
				{
					new Float:damage = random_float(get_pcvar_float(cvar_mindamage), get_pcvar_float(cvar_maxdamage))
					new Float:vOrigin[3];
					pev(iVictim, pev_origin, vOrigin);
					Create_Blood(vOrigin, iBlood[0], iBlood[1], 76, 13); 
					set_pev(iVictim, pev_velocity, {0.0,0.0,-100.0});
					ExecuteHamB(Ham_TakeDamage, iVictim, iEntity, iAttacker, damage, DMG_BLAST);
				}
			}
...
public HamHook_Touch(const iEntity, const iOther)
{
	...
	if (equal(Classname, MINE_CLASSNAME))
	{
		if (is_user_alive(iOther) && zp_get_user_zombie(iOther))
		{
			...
			set_pev(iEntity, pev_fuser1, get_gametime() + get_pcvar_float(cvar_explodedelay));
